...
def test_plugin_registry_is_additive(monkeypatch):
    import densevideo_qwen_dual_plugin.models as plugin_models
    from lmms_eval.models import AVAILABLE_MODELS

    assert plugin_models.AVAILABLE_MODELS == {
        "qwen2_5_vl_dual_route": "Qwen2_5_VL_DualRoute",
        "qwen2_5_vl_dual_route_floor": "Qwen2_5_VL_DualRouteFloor",
    }
    assert "qwen2_5_vl_dual_route" not in AVAILABLE_MODELS
    assert "qwen2_5_vl_dual_route_floor" not in AVAILABLE_MODELS
...
def test_request_route_is_cleared_after_failure(monkeypatch):
    ...
    request = SimpleNamespace(
        args=("What text is extracted by OCR in the entire video?",)
    )
