...
from lmms_eval.models.llava_hf import (
...
class LlavaHfGatedEmbeddingsTest(unittest.TestCase):
    ...
    def test_motion_all_keep_frames_use_exact_native_conv(self):
        ...
        gated.set_video_frames(4)
        conv_calls = []
        hook = gated.patch_embedding.register_forward_hook(
            lambda *_: conv_calls.append(True)
        )
    ...
    def test_linear_consistent_reproduces_old_all_keep_kernel_and_calls_conv_once(self):
        ...
        patches = torch.nn.functional.unfold(
            ...
            padding=(
                0
                if self.native.patch_embedding.padding == "valid"
                else self.native.patch_embedding.padding
            ),
        ...
        expected = expected + self.native.position_embedding(
            self.native.position_ids
        )
    ...
    def test_invalid_projection_or_refresh_arguments_fail_before_model_load(self):
        ...
        with mock.patch(
            "lmms_eval.models.llava_hf.AutoConfig.from_pretrained"
        ) as load_config, mock.patch(
            "lmms_eval.models.llava_hf.Accelerator"
        ) as load_accelerator:
            with self.assertRaisesRegex(ValueError, "gate_projection_mode"):
...
class LlavaHfPromptRouterTest(unittest.TestCase):
    def test_classifier_uses_only_the_first_sentence(self):
        self.assertEqual(
            classify_densevideo_lpm_first_sentence(
                "  What   subtitles appear in the entire video clip-1? "
                "What text is extracted by OCR later?"
            ),
            "subtitle",
        )
        self.assertEqual(
            classify_densevideo_lpm_first_sentence(
                "WHAT TEXT IS EXTRACTED BY OCR IN THE ENTIRE VIDEO clip-1? Mention subtitles too."
            ),
            "ocr",
        )
        self.assertEqual(
            classify_densevideo_lpm_first_sentence(
                "Describe the video. What text is extracted by OCR in the entire video?"
            ),
            "unknown",
        )
    ...
    def test_route31_changes_only_subtitle_token_cap_on_pyav(self):
        ...
        subtitle = resolve_video_request_plan(
            "What subtitles appear in the entire video clip?", **common
        )
        ocr = resolve_video_request_plan(
            "What text is extracted by OCR in the entire video clip?", **common
        )
...
class LlavaHfVideoDecodeBackendTest(unittest.TestCase):
    ...
    def test_pyav_seek_caps_short_video_and_reference(self):
        ...
        with mock.patch("lmms_eval.models.llava_hf.av.open", return_value=container), mock.patch(
            "lmms_eval.models.llava_hf.read_video_pyav_seek_uniform",
    ...
    def test_frozen_decord_uses_validated_post_resize_frames_without_live_decode(self):
        ...
        model._frozen_decord_cache.load.assert_called_once_with(
            "/dataset/videos/clip.mp4"
        )
    ...
    def test_native_control_reports_reference_projection_ratio(self):
        ...
        model._model = SimpleNamespace(
            model=SimpleNamespace(vision_tower=SimpleNamespace(vision_model=vision_model))
        )
