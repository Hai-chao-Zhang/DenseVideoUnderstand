...
class HighMotionFrameAlignmentTests(unittest.TestCase):
    ...
    def _doc(labels):
        return {
            ...
            "question": (
                f"Move an object.\n\nWe consider all {len(labels)} frames in temporal order. "
                "For every frame t = 1…{F}, locate the hand.\n\n"
                f"Answer format: return a comma-separated list of {len(labels)} region names."
            ),
        }
    ...
    def test_metrics_use_the_same_sampled_reference(self):
        ...
        self.assertEqual(result["grid_acc"]["ref_grid_len"], 8)
        self.assertEqual(result["grid_acc"]["pred_grid_len"], 8)
        self.assertEqual(result["grid_acc"]["score"], 1.0)
        self.assertEqual(result["grid_ade"]["score"], 0.0)
        self.assertEqual(result["grid_fde"]["score"], 0.0)
        self.assertEqual(result["grid_transition_acc"]["score"], 1.0)
