...
This module is deliberately an external lmms-eval plugin.  Loading it through
``LMMS_EVAL_PLUGINS=densevideo_qwen_dual_plugin`` leaves both the production
Qwen wrapper and lmms-eval's built-in model registry byte-for-byte unchanged.
...
from lmms_eval.models.qwen2_5_vl import (
...
class DualRouteGatedQwenVisionPatchEmbed(GatedQwenVisionPatchEmbed):
    ...
    def gate_policy(self, value: object) -> None:
        ...
        if policy not in {"motion", "random", "all"}:
            raise ValueError(
                f"Unsupported gate_policy={value}. Expected motion|random|all."
            )
...
class RouteFloorGatedQwenVisionPatchEmbed(DualRouteGatedQwenVisionPatchEmbed):
    ...
    def _native_floor_fallback(
        ...
        *,
        ...
        self.last_keep_flat = torch.ones(
            out.shape[0], dtype=torch.bool, device=out.device
        )
        self.last_diff_score_flat = torch.ones(
            out.shape[0], dtype=torch.float32, device=out.device
        )
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        for grid_t, grid_h, grid_w in rows:
            ...
            keep = torch.zeros(
                (grid_t, spatial_tokens), dtype=torch.bool, device=segment.device
            )
            scores = torch.zeros(
                (grid_t, spatial_tokens), dtype=torch.float32, device=segment.device
            )
            keep[0] = True
            scores[0] = 1.0
            if grid_t > 1:
                if self.gate_policy == "random":
                    generator_device = (
                        segment.device if segment.device.type == "cuda" else "cpu"
                    )
        ...
        for segment, keep, _scores, segment_start, segment_tokens in segments:
            ...
            out_segment[0] = self.proj(first_tubes.to(dtype=target_dtype)).view(
                -1, self.embed_dim
            )
            for t_idx in range(1, grid_t):
                ...
                if selected.all():
                    ...
                    out_segment[t_idx] = self.proj(
                        tubes.to(dtype=target_dtype)
                    ).view(-1, self.embed_dim)
                elif selected_count:
                    out_segment[t_idx, selected] = F.linear(
                        segment[t_idx, selected], weight_flat, bias
                    )
...
@register_model("qwen2_5_vl_dual_route")
class Qwen2_5_VL_DualRoute(Qwen2_5_VL):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.prompt_router != DENSEVIDEO_LPM_PROMPT_ROUTER:
            raise ValueError(
                "qwen2_5_vl_dual_route requires "
        ...
        if not _as_bool(kwargs.get("use_custom_video_loader", False)):
            raise ValueError(
                "qwen2_5_vl_dual_route requires use_custom_video_loader=True"
            )
    ...
    def _activate_route(self, route: str) -> None:
        ...
        if self.profiling:
            ...
            print(
                "[DENSE_ROUTE_GATE] "
                f"prompt_router={self.prompt_router} "
                f"question_route={route} "
                f"gate_policy={self._dual_route_patch_embed.gate_policy} "
                f"gate_diff_threshold={threshold_label}",
                flush=True,
            )
    ...
    def generate_until(self, requests: Iterable[object]):
        ...
        for request in requests:
            route = classify_densevideo_lpm_first_sentence(
            ...
            try:
                ...
                if len(result) != 1:
                    raise RuntimeError(
                        "frozen Qwen wrapper returned a non-singleton response "
                        "for a request-local dual-route call"
                    )
...
@register_model("qwen2_5_vl_dual_route_floor")
